...
inline asio::awaitable<void> serve(asio::generic::stream_protocol::socket socket) {
    ...
    try {
        ...
        log::info("[client {:#0{}x}] starting connection", id, sizeof(id) + 2);

        co_await endpoint.run();
    } catch (std::exception & ex) {
        log::info("[client {:#0{}x}] closing connection: {}", id, sizeof(id) + 2, ex.what());
    }
}
